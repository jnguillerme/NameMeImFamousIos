...
/*    MosquittoMessage *mosq_msg = [[MosquittoMessage alloc] init];
  ...
  */
    ...
    [[client delegate] didReceiveMessage:[[NSString alloc] initWithBytes:message->payload
                                                                  length:message->payloadlen
                                                                encoding:NSUTF8StringEncoding] forTopic:[NSString stringWithUTF8String: message->topic]];
...
- (MosquittoClient*) initWithClientId: (NSString*) clientId andCleanSession:(BOOL)theCleanSessionFlag{
    if ((self = [super init])) {
        ...
        [self setCleanSession: theCleanSessionFlag]; //NOTE: this isdisable clean to keep the broker remember this client
